...
const accountSchema = new mongoose.Schema(
  {
    ...
    // Interest Rate (mainly for savings accounts)
    interestRate: {
      type: Number,
      default: function() {
...
accountSchema.methods.canTransact = function(amount, transactionType = 'debit') {
  ...
  if (transactionType === 'debit') {
    ...
    if (this.balance - amount < this.minimumBalance) {
      return { 
        canTransact: false, 
        reason: `Insufficient balance. Minimum balance required: ₹${this.minimumBalance}` 
      };
    }
